...
mongoose.connect("mongodb+srv://PeenkLion:PL_Mongo_123_A@astrofn.fz4xq.mongodb.net/myFirstDatabase?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true, useFindAndModify: true }, async e => {
    if (e) throw e
})
...
server.listen(Port,() => {
    ...
    prompt.get(['Username', "Password"], async (err, result) => {
        ...
        if (User1 == null){
            ...
            var user = new User({ id: id, displayName: result.Username, email: result.Username + "@astrofn.Dev", password: bcrypt.hashSync(result.Password, bcrypt.genSaltSync(10)) })
            user.save()
            ...
            athena.save()
            console.log("User Created! your email is: " + result.Username + "@astrofn.Dev")
        }
